...
const PANEL1_PATH = path.join(DATA_DIR, 'panel1.json');       // panel config 1
const PANEL2_PATH = path.join(DATA_DIR, 'panel2.json');       // panel config 2
...
// load slash command files from /commands
...
async function fetchGuildData() {
  try {
    ...
    const categories = guild.channels.cache.filter(c=>c.type===4).map(c=>({ id:c.id, name:c.name })); // 4 category
    const voices = guild.channels.cache.filter(c=>c.type===2).map(c=>({ id:c.id, name:c.name }));    // 2 voice
    const text   = guild.channels.cache.filter(c=>c.type===0).map(c=>({ id:c.id, name:'#'+c.name })); // 0 text
...
client.login(process.env.DISCORD_TOKEN).then(()=>{
  startKickWatcher(client); // start Kick checker after login
});
